...
// Depends on: state.js, helpers.js, navigation.js, npi-constants.js, npi.js, pfmea.js
// ═══════════════════════════════════
...
function stepNumConflict(pfd, num) { return pfd.some(s => s.stepNum === num) }

function stepRowHTML(s, oi, p) {
  const ctqBadges = (s.ctqIds || []).map(cid => { const ci = p.ctq.findIndex(c => c.id === cid); return ci >= 0 ? `<span class="ctq-pick-item" onclick="npi.apqp.openCtqPick(${oi})">C${ci + 1}</span>` : '' }).join('')
  const pfCnt = p.pfmea.filter(r => r.pfdId === s.id).length
  const pills = (s.bomRefs || []).map(ref => { const bt = p.bom[ref.bomType]; if (!bt) return ''; const item = bt.find(x => x.id === ref.itemId); if (!item) return ''; const t = BOM_TYPES[ref.bomType]; const name = item.desc || (item.pn || item.toolId || item.equipId || '?'); return `<span class="res-pill ${t.pc}" onclick="npi.apqp.delBomRef('${s.id}','${ref.bomType}','${ref.itemId}')" title="Click to remove">${t.icon} ${esc(name.length > 18 ? name.slice(0, 18) + '…' : name)}${item.isAaw ? ' <span class="flag flag-aaw" style="font-size:9px">AAW</span>' : ''}</span>` }).join('')
  return `<div class="step-row" id="pfd-row-${s.id}"><div class="step-main-row"><div class="step-num-cell"><div class="step-num-badge">${s.stepNum}</div><div style="display:flex;flex-direction:column;gap:2px"><button class="mini-btn" onclick="npi.apqp.openInsert(${oi})">＋</button><button class="mini-btn danger" onclick="npi.apqp.delPFD('${s.id}')">×</button></div></div><div class="step-body"><div class="step-fields"><div class="step-field f-op"><input class="cell-edit" value="${esc(s.op)}" onchange="npi.apqp.updPFD('${s.id}','op',this.value)" placeholder="Operation" style="font-weight:600"></div><div class="step-field f-detail"><textarea class="cell-edit" rows="2" onchange="npi.apqp.updPFD('${s.id}','detail',this.value)" placeholder="Method / notes…">${esc(s.detail)}</textarea></div><div class="step-field f-ctq"><div class="ctq-pick">${ctqBadges}${p.ctq.length > 0 ? `<span class="ctq-pick-add" onclick="npi.apqp.openCtqPick(${oi})">＋ CTQ</span>` : ''}</div></div><div class="step-field f-pfmea">${pfCnt > 0 ? `<span class="tag tag-amber">${pfCnt} FMEA</span>` : '<span style="font-size:11px;color:var(--muted)">—</span>'}</div></div></div></div><div class="step-resources">${pills}<button class="res-add-btn" onclick="npi.apqp.openBomPick('${s.id}')">＋ Resource</button></div></div>`
}
...
// CTQ
// ══════════════════════════════════════
npi.apqp.renderCTQ = function() {
  const p = prog()
  const rows = p.ctq.map((r, i) => `<tr>
    <td style="text-align:center"><span class="tag tag-ctq">C${i + 1}</span></td>
    <td><textarea class="cell-edit" rows="2" onchange="npi.apqp.updCTQ(${i},'req',this.value)" placeholder="CTQ requirement">${esc(r.req)}</textarea></td>
    <td><input class="cell-edit mono" value="${esc(r.spec)}" onchange="npi.apqp.updCTQ(${i},'spec',this.value)" placeholder="e.g. 50±0.05mm" style="width:100%"></td>
    <td><input class="cell-edit" value="${esc(r.testMethod || '')}" onchange="npi.apqp.updCTQ(${i},'testMethod',this.value)" placeholder="e.g. CMM, Gauge, Visual" style="width:100%"></td>
    <td><select class="cell-edit" onchange="npi.apqp.updCTQ(${i},'source',this.value)" style="width:100%">${['Customer Spec', 'OEM Data', 'Internal Standard', 'Regulatory', 'Drawing'].map(o => `<option${r.source === o ? ' selected' : ''}>${o}</option>`).join('')}</select></td>
    <td><select class="cell-edit" onchange="npi.apqp.updCTQ(${i},'oos_action',this.value)" style="width:100%">${['Repair', 'Replace', 'Scrap', 'Review', 'TBD'].map(o => `<option${r.oos_action === o ? ' selected' : ''}>${o}</option>`).join('')}</select></td>
    <td><div class="ctq-agreed">
      <input type="checkbox" ${r.customerAgreed ? 'checked' : ''} onchange="npi.apqp.updCTQ(${i},'customerAgreed',this.checked);render()" title="Customer has agreed this CTQ method and out-of-spec plan">
      <span class="ctq-agreed-label" style="color:${r.customerAgreed ? 'var(--green)' : 'var(--muted)'}">${r.customerAgreed ? 'AGREED' : '—'}</span>
    </div></td>
    <td style="text-align:center"><button class="del-btn" onclick="npi.apqp.delCTQ(${i})">×</button></td>
  </tr>`).join('')
  return `<div class="sec-head"><div><div class="sec-eyebrow">Step 01</div><div class="sec-title">CTQ Matrix</div><div class="sec-desc">Critical-to-Quality requirements — source of truth for PFD, PFMEA and Control Plan.</div></div><div class="sec-actions"><button class="btn btn-primary btn-sm" onclick="npi.apqp.addCTQ()">＋ Add CTQ</button></div></div>
...
// PFD
// ══════════════════════════════════════
npi.apqp.renderPFD = function() {
  const p      = prog()
  const sorted = sortedPfd(p.pfd)
  const ribbon = sorted.filter(s => s.type !== 'group').map((s, i, arr) =>
    `<div class="flow-node${s.type === 'sub' ? ' is-sub' : ''}" onclick="npi.apqp.scrollToPfd('${s.id}')"><div class="flow-node-num">${s.stepNum}</div><div class="flow-node-name">${esc(s.op) || '—'}</div></div>${i < arr.length - 1 ? '<div class="flow-arrow">→</div>' : ''}`
  ).join('')
  ...
  <div class="sec-actions"><button class="btn btn-ghost btn-sm" onclick="npi.apqp.openInsert(null,'group')">＋ Sub-assembly</button><button class="btn btn-primary btn-sm" onclick="npi.apqp.addMainStep()">＋ Add Step</button></div></div>
...
npi.apqp.openCtqPick = function(oi) {
  ...
  document.getElementById('ctqPickList').innerHTML = p.ctq.length === 0
    ...
    : p.ctq.map((c, i) => `<label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;padding:7px;border-radius:5px;border:1px solid var(--line);background:var(--white)" onmouseover="this.style.background='var(--blue-pale)'" onmouseout="this.style.background='var(--white)'"><input type="checkbox" ${ctqPickSelected.includes(c.id) ? 'checked' : ''} onchange="npi.apqp.tCP('${c.id}',this.checked)" style="margin-top:2px;accent-color:var(--blue)"><div><div style="display:flex;align-items:center;gap:6px"><span class="tag tag-ctq">C${i + 1}</span><span style="font-size:12px;font-weight:600">${esc(c.req || 'Unnamed')}</span></div><div style="font-size:11px;color:var(--muted);font-family:'IBM Plex Mono',monospace;margin-top:1px">${esc(c.spec)}</div></div></label>`).join('')
...
npi.apqp.renderCP = function() {
  ...
  const rows = p.cp.map((r, i) => {
    ...
    const rc   = rpn >= RPN_CRITICAL ? 'rpn-hi' : rpn >= RPN_HIGH ? 'rpn-md' : 'rpn-lo'
    ...
    return `<tr><td class="w100"><span class="tag tag-step" style="font-size:10px">${sl}</span></td>
      ...
      <td><input class="cell-edit" value="${esc(r.char)}" onchange="npi.apqp.updCP(${i},'char',this.value)" placeholder="Characteristic"></td>
      <td class="w80"><select class="cell-edit" onchange="npi.apqp.updCP(${i},'type',this.value)">${['Product', 'Process', 'Dimensional', 'Functional', 'Visual'].map(o => `<option${r.type === o ? ' selected' : ''}>${o}</option>`).join('')}</select></td>
      <td class="w110"><input class="cell-edit mono" value="${esc(r.spec)}" onchange="npi.apqp.updCP(${i},'spec',this.value)" placeholder="Spec"></td>
      <td class="w110"><input class="cell-edit" value="${esc(r.method)}" onchange="npi.apqp.updCP(${i},'method',this.value)" placeholder="Method"></td>
      <td class="w60"><input class="cell-edit" value="${esc(r.freq)}" onchange="npi.apqp.updCP(${i},'freq',this.value)" placeholder="100%"></td>
      <td class="w70"><input class="cell-edit" value="${esc(r.resp)}" onchange="npi.apqp.updCP(${i},'resp',this.value)" placeholder="Who"></td>
      <td><textarea class="cell-edit" rows="2" onchange="npi.apqp.updCP(${i},'reaction',this.value)" placeholder="Reaction plan">${esc(r.reaction)}</textarea></td>
      <td class="w50"><div style="display:flex;flex-wrap:wrap;gap:2px">${ctqs || '—'}</div></td>
      <td class="w28 ctr"><button class="del-btn" onclick="npi.apqp.delCP(${i})">×</button></td></tr>`
  }).join('')
  const syncBanner = miss.length > 0
    ? `<div style="background:var(--amber-pale);border:1px solid var(--amber-mid);border-radius:6px;padding:10px 14px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;gap:12px"><span style="font-size:13px;color:var(--amber)">⚠ ${miss.length} PFMEA effect${miss.length !== 1 ? 's' : ''} not in control plan.</span><button class="btn btn-sm" style="background:var(--amber);color:white;border:none" onclick="npi.apqp.syncFromPFMEA()">Auto-populate from PFMEA</button></div>`
    : ''
  ...
  <div class="sec-actions"><button class="btn btn-ghost btn-sm" onclick="npi.apqp.syncFromPFMEA()">Sync from PFMEA</button><button class="btn btn-primary btn-sm" onclick="npi.apqp.addCP()">＋ Add Row</button></div></div>
  ${syncBanner}
  ...
  <button class="add-row" onclick="npi.apqp.addCP()">＋ Add Row</button></div>`
}
