...
function stepRowHTML(s, oi, p) {
  const ctqBadges = (s.ctqIds || []).map(cid => {
    ...
    return ci >= 0 ? `<span class="ctq-pick-item" onclick="npi.pfd.openCtqPick(${oi})">C${ci + 1}</span>` : ''
  }).join('')
  ...
  const pills = (s.bomRefs || []).map(ref => {
    ...
    return `<span class="res-pill ${t.pc}" onclick="npi.pfd.delBomRef('${s.id}','${ref.bomType}','${ref.itemId}')" title="Click to remove">${t.icon} ${esc(name.length > 18 ? name.slice(0, 18) + '…' : name)}${item.isAaw ? ' <span class="flag flag-aaw" style="font-size:9px">AAW</span>' : ''}</span>`
  }).join('')

  return `<div class="step-row" id="pfd-row-${s.id}"><div class="step-main-row"><div class="step-num-cell"><div class="step-num-badge">${s.stepNum}</div><div style="display:flex;flex-direction:column;gap:2px"><button class="mini-btn" onclick="npi.pfd.openInsert(${oi})">＋</button><button class="mini-btn danger" onclick="npi.pfd.del('${s.id}')">×</button></div></div><div class="step-body"><div class="step-fields"><div class="step-field f-op"><input class="cell-edit" value="${esc(s.op)}" onchange="npi.pfd.upd('${s.id}','op',this.value)" placeholder="Operation" style="font-weight:600"></div><div class="step-field f-detail"><textarea class="cell-edit" rows="2" onchange="npi.pfd.upd('${s.id}','detail',this.value)" placeholder="Method / notes…">${esc(s.detail)}</textarea></div><div class="step-field f-ctq"><div class="ctq-pick">${ctqBadges}${p.ctq.length > 0 ? `<span class="ctq-pick-add" onclick="npi.pfd.openCtqPick(${oi})">＋ CTQ</span>` : ''}</div></div><div class="step-field f-pfmea">${pfCnt > 0 ? `<span class="tag tag-amber">${pfCnt} FMEA</span>` : '<span style="font-size:11px;color:var(--muted)">—</span>'}</div></div></div></div><div class="step-resources">${pills}<button class="res-add-btn" onclick="npi.pfd.openBomPick('${s.id}')">＋ Resource</button></div></div>`
}
...
npi.pfd.render = function() {
  ...
  const ribbon = sorted.filter(s => s.type !== 'group').map((s, i, arr) =>
    `<div class="flow-node${s.type === 'sub' ? ' is-sub' : ''}" onclick="npi.pfd.scrollTo('${s.id}')"><div class="flow-node-num">${s.stepNum}</div><div class="flow-node-name">${esc(s.op) || '—'}</div></div>${i < arr.length - 1 ? '<div class="flow-arrow">→</div>' : ''}`
  ).join('')
  ...
  while (i < sorted.length) {
    ...
    if (s.type === 'group') {
      ...
      body += `<div class="step-row" id="pfd-row-${s.id}"><div class="sub-group-header" onclick="npi.pfd.toggleGroup('${s.id}')"><span style="font-size:10px;color:var(--purple);display:inline-block;${col ? '' : 'transform:rotate(90deg)'}">▶</span><span class="tag tag-sub">${s.stepNum}</span><span style="font-size:13px;font-weight:600;color:var(--purple)">${esc(s.op) || 'Sub-assembly Group'}</span><span style="font-size:11px;color:#9b74cc;margin-left:auto">${ch.length} step${ch.length !== 1 ? 's' : ''}</span><button class="del-btn" style="margin-left:8px" onclick="npi.nav.stopEvent(event);npi.pfd.del('${s.id}')">×</button></div><div class="sub-group-body${col ? ' collapsed' : ''}"> ${ch.map(c => stepRowHTML(c.s, c.oi, p)).join('')}</div></div>`
      i = j
    } else { body += stepRowHTML(s, oi, p); i++ }
    body += `<div class="insert-row"><button class="insert-btn" onclick="npi.pfd.openInsert(${oi})">＋ after ${s.stepNum}</button></div>`
  }
  ...
  <div class="sec-actions"><button class="btn btn-ghost btn-sm" onclick="npi.pfd.openInsert(null,'group')">＋ Sub-assembly</button><button class="btn btn-primary btn-sm" onclick="npi.pfd.addMainStep()">＋ Add Step</button></div></div>
  ...
  <button class="add-row" onclick="npi.pfd.addMainStep()">＋ Add Process Step</button></div>
...
npi.pfd.addMainStep = function() { npi.data.pfd.addMainStep(); render() }
...
npi.pfd.del = function(sid) { npi.data.pfd.del(sid); render() }
...
npi.pfd.toggleGroup = function(key) { npi.data.pfd.toggleGroup(collapsedGroups, key); render() }
npi.pfd.delBomRef = function(sid, bt, iid) { npi.data.pfd.delBomRef(sid, bt, iid); render() }
...
npi.pfd.openCtqPick = function(oi) {
  ...
  document.getElementById('ctqPickList').innerHTML = p.ctq.length === 0
    ...
    : p.ctq.map((c, i) => `<label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;padding:7px;border-radius:5px;border:1px solid var(--line);background:var(--white)" onmouseover="this.style.background='var(--blue-pale)'" onmouseout="this.style.background='var(--white)'"><input type="checkbox" ${ctqPickSelected.includes(c.id) ? 'checked' : ''} onchange="npi.pfd.toggleCtqPick('${c.id}',this.checked)" style="margin-top:2px;accent-color:var(--blue)"><div><div style="display:flex;align-items:center;gap:6px"><span class="tag tag-ctq">C${i + 1}</span><span style="font-size:12px;font-weight:600">${esc(c.req || 'Unnamed')}</span></div><div style="font-size:11px;color:var(--muted);font-family:'IBM Plex Mono',monospace;margin-top:1px">${esc(c.spec)}</div></div></label>`).join('')
...
npi.pfd.refreshBomPickModal = function(p, filterId, listId, activeFilter) {
  ...
  filterEl.innerHTML = `<button class="bom-filter-btn${activeFilter === 'all' ? ' active' : ''}" onclick="npi.pfd.setBomFilter('all','${filterId}','${listId}')">All (${total})</button>` +
    types.map(([k, t]) => `<button class="bom-filter-btn${activeFilter === k ? ' active' : ''}" onclick="npi.pfd.setBomFilter('${k}','${filterId}','${listId}')">${t.icon} ${t.label} (${p.bom[k].length})</button>`).join('')
  ...
  types.forEach(([k, t]) => {
    ...
    p.bom[k].forEach(item => {
      ...
      items.push(`<div class="bom-pick-item${bomPickSelected.includes(key) ? ' selected' : ''}" onclick="npi.pfd.toggleBomPick('${key}',this)"><input type="checkbox" ${bomPickSelected.includes(key) ? 'checked' : ''} onchange="npi.pfd.toggleBomPick('${key}',this.closest('.bom-pick-item'))"><div class="bom-pick-info"><div class="bom-pick-name">${t.icon} ${esc(name || 'Unnamed')}</div><div class="bom-pick-meta">${esc(meta)}</div><div style="display:flex;gap:3px;margin-top:3px;flex-wrap:wrap">${flags.join('')}</div></div><span class="tag" style="font-size:9px;background:var(--bg);color:var(--muted);border:1px solid var(--line);align-self:flex-start">${t.label}</span></div>`)
    })
  })
