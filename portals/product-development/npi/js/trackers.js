...
npi.tracker.renderRisks = function() {
  ...
  const rows = p.risks.map((r, i) => {
    ...
    return `<tr class="${score >= 12 && r.status !== 'Closed' ? 'row-hi' : ''}">
      ...
      <td style="text-align:center"><input type="number" class="cell-edit mono" min="1" max="5" value="${r.lik || 1}" onchange="npi.tracker.updRisk(${i},'lik',+this.value);npi.tracker.refreshRS(${i})" style="width:100%;text-align:center"></td>
      <td style="text-align:center"><input type="number" class="cell-edit mono" min="1" max="5" value="${r.imp || 1}" onchange="npi.tracker.updRisk(${i},'imp',+this.value);npi.tracker.refreshRS(${i})" style="width:100%;text-align:center"></td>
