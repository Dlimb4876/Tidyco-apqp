...
window.capParseDateOnlyLocal = function(value) {
...
window.capGetHolidayDaysInRange = function(memberId, rangeStart, rangeEnd, holidaysArray, bankHolSet) {
...
window.capGetProductBatchCountInRange = function(product, rangeStart, rangeEnd) {
...
window.capGetProductBatchesInRange = function(product, rangeStart, rangeEnd) {
...
window.capGetProductSupportHoursForBatch = function(product, batch, monthStart, fallbackHoursPerWeek, supportRateResolver) {
...
window.capCalculateMonthData = function(monthKey, teamArray, tasksArray, productsArray, holidaysArray, options) {
  ...
  productsArray.forEach(product => {
    ...
    const overlappingBatches = window.capGetProductBatchesInRange(product, monthStart, monthEnd);
    overlappingBatches.forEach(batch => {
      const supportPerBatch = window.capGetProductSupportHoursForBatch(
        product,
        batch,
        monthStart,
...
window.capGetWeekRange = function(monthKey, weekCount) {
  const [year, month] = monthKey.split('-').map(Number);
  const monthStart = new Date(year, month - 1, 1);
