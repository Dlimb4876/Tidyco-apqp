...
// ── Capacity Defaults ───────────────────────────────────────
window.CAP_HOURS_PER_DAY = 8;
window.CAP_DEFAULT_HOURS_PER_WEEK = window.CAP_HOURS_PER_DAY * 5;

window.capGetHoursPerWeek = function(hoursPerWeek) {
  const parsed = parseFloat(hoursPerWeek);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : window.CAP_DEFAULT_HOURS_PER_WEEK;
};

window.capGetDailyHours = function(hoursPerWeek, utilisationPercent) {
...
window.getBankHolidaysForYear = function(year) {
  ...
  if (!window.capBankHolidaysCache) {
    window.capBankHolidaysCache = {};
  }

  if (window.capBankHolidaysCache[year]) {
    return window.capBankHolidaysCache[year];
  }
  ...
  window.capBankHolidaysCache[year] = holidays;
  return holidays;
};

// Runtime-safe legacy aliases remain until the old ME shared files are deleted.
if (typeof window.meGetHoursPerWeek !== 'function') {
  window.meGetHoursPerWeek = window.capGetHoursPerWeek;
}

if (typeof window.meGetDailyHours !== 'function') {
  window.meGetDailyHours = window.capGetDailyHours;
}

if (typeof window.meFormatDate !== 'function') {
  window.meFormatDate = window.formatDate;
}

if (typeof window.meGetBankHolidaysForYear !== 'function') {
  window.meGetBankHolidaysForYear = window.getBankHolidaysForYear;
}

if (typeof window.meGetMonthLabel !== 'function') {
  window.meGetMonthLabel = function(monthKey) {
    const labelParts = window.getMonthLabel(monthKey);
    return Array.isArray(labelParts) ? labelParts.join(' ') : labelParts;
  };
}
