# Product Development

## What this area is for
Product Development manages NPI delivery from early project setup through APQP maturity and gate sign-off.
It connects engineering intent, process definition, risk control, and release readiness in one workflow.

## End-to-end workflow
1. Start in NPI Projects and open the target project.
2. Build APQP content (CTQ, PFD, PFMEA, Control Plan, BOM, Timing).
3. Drive risks and actions to closure.
4. Complete gate evidence and sign-offs.

## How this links to other systems
- Product Management provides core product, family, and part data.
- Operations consumes risk and action signals.
- MCS handles controlled manufacturing changes when process or design updates are needed.

## Related
- [NPI Projects](./10-npi-projects.md)
- [APQP Gates](./110-gates.md)
- [Product Management Overview](../product-management/00-overview.md)
