...
export async function pmLoadRelationalProductSupportAllocations() {
  try {
    const { data, error } = await supabase.from('pm_product_support_allocations').select('*')
    if (error) {
      safeWarn('pmLoadRelationalProductSupportAllocations error:', error)
      return []
    }
    return (data || []).map(row => ({
      id: row.id,
    ...
    safeWarn('pmLoadRelationalProductSupportAllocations exception:', err)
    return []
  }
}

export async function pmSaveProductSupportAllocationSet(productId, effectiveDate, rows) {
  try {
    ...
    if (prevDay) {
      ...
      const { error: closeError } = await supabase
        .from('pm_product_support_allocations')
        .update({ end_date: endDateStr, updated_at: new Date().toISOString() })
        .eq('product_id', productId)
        .is('end_date', null)

      if (closeError) {
        safeWarn('pmSaveProductSupportAllocationSet close error:', closeError)
        return false
      }
    }
    ...
    const { error: insertError } = await supabase
      ...
      .insert(payload)
    if (insertError) {
      safeWarn('pmSaveProductSupportAllocationSet insert error:', insertError)
      return false
    }
    return true
  } catch (err) {
    safeWarn('pmSaveProductSupportAllocationSet exception:', err)
    return false
  }
}

export async function pmDeleteProductSupportAllocation(allocationId) {
  try {
    const { error } = await supabase
      ...
      .eq('id', allocationId)
    if (error) {
      safeWarn('pmDeleteProductSupportAllocation error:', error)
      return false
    }
    return true
  } catch (err) {
    safeWarn('pmDeleteProductSupportAllocation exception:', err)
    return false
  }
}
