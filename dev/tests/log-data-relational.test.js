...
describe('log-data-relational', () => {
  ...
  it('saves Logistics tasks to log_tasks', async () => {
    ...
    const result = await window.logSaveTaskRelational('user-1', {
      id: 'task-1',
      name: 'Move stock',
      category: 'Support',
      assigneeId: 'member-1',
  ...
  it('deletes holidays only from log_holidays', async () => {
    ...
    const ok = await window.logDeleteHolidayRelational('holiday-1');
  ...
  it('persists product support history to log_product_support_history', async () => {
    ...
    const ok = await window.logSaveProductSupportHistoryRelational('user-1', [{
      id: 'hist-1',
      productId: 'prod-1',
      hoursPerWeek: 3,
