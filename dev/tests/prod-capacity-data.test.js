...
const {
  ...
  prodCapDataInit,
  prodCapLoadUtilization,
  prodCapDataSetStaff
} = await import(resolve(__dirname, '../portals/capacity/production/js/prod-capacity-data.js'))

describe('Production capacity calculations', () => {
  beforeEach(() => {
    global.prodCapUtilizationFactor = 1
    global.prodState.products = []
  ...
  test('uses a 40-hour week baseline for available capacity', () => {
    const originalGetStaff = global.prodCapDataGetStaff
    global.prodCapDataGetStaff = () => 1

    // 21 working days in Jan 2026 => 40 * (21/5) = 168 hours.
    expect(prodCapAvailableHours('Unit 2', 2026, 1)).toBeCloseTo(168, 6)

    global.prodCapDataGetStaff = originalGetStaff
  })
