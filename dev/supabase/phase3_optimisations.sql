...
-- The hub loads projects ordered by updated_at DESC.
-- This partial index makes that query a fast index-only scan.
CREATE INDEX IF NOT EXISTS idx_projects_updated_at
  ON projects (updated_at DESC);

-- ── 2. NPI relational query — project_id indexes ───────────────
-- Every NPI data load filters all 15 tables by project_id.
...
CREATE INDEX IF NOT EXISTS idx_npi_pfd_steps_prog    ON npi_pfd_steps    (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_pfmea_modes_prog  ON npi_pfmea_modes  (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_pfmea_effects_prog ON npi_pfmea_effects (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_pfmea_causes_prog  ON npi_pfmea_causes  (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_pfmea_history_prog ON npi_pfmea_history (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_control_plan_prog  ON npi_control_plan  (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_bom_items_prog     ON npi_bom_items     (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_bom_kits_prog      ON npi_bom_kits      (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_bom_kit_items_prog ON npi_bom_kit_items (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_gates_prog         ON npi_gates         (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_gate_sigs_prog     ON npi_gate_sigs     (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_actions_prog       ON npi_actions       (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_risks_prog         ON npi_risks         (project_id);
CREATE INDEX IF NOT EXISTS idx_npi_gantt_rows_prog    ON npi_gantt_rows    (project_id);

-- ── 3. Cascade deletes — NPI relational data ─────────────────────
-- Ensures that deleting a project from the `projects` table
-- automatically removes all associated NPI rows at the DB level.
-- This is a safety net in addition to the client-side cascade
-- already implemented in Phase 2 (npiRelDeleteAllForProject).
--
...
ALTER TABLE npi_ctq
  ...
  FOREIGN KEY (project_id) REFERENCES projects(prog_id) ON DELETE CASCADE;
